...
def inputClassifier(inputText):  
    ...
    2) Customer wants to know when their vehicle will be charged. Classify it as ```When``` \
    3) Customer wants to know why their vehicle is not charging. Classify it as ```Not Charging``` \
    5) If it is not one of the above, classify it as ```Other```
